...
There's one other thing we can look at as well. What is the frequency of customers who churn based on the numeric variables? For instance, based on monthy charges is there a higher frequency of customer churn at a specific value? Let's find out. 
...
There are some big conclusions we can draw from this. Customers with less tenure are more likely to leave, which should be unsurprising. People who have stuck around will continue to stick around. We also see higher values of monthly charges resulting in higher frequencies of customer churn. It's possible there is some break point in monthly payments where customers are no longer willing to continue service. Lastly, customers who have spent less total on telco's services are more likely to leave. This is similar to tenure. I would expect these three to be good predictors of churn as they seem intuitively useful.
...
- **Accuracy:** The most intuitive, shows how often our models predictions were accurate. Our model had an accuracy of around **.75**, which is roughly the same as our null model. 

- **Precision:** This show us our number of true positive with relation to the number of false positives we got. Our model has quite a respectable value at around **.90** which indicates out of all our positive guesses, 90% of them were on the mark. 

- **Recall:** This is our true positive rate. A high sensitivity score indicates we had few false negatives and were able to capture almost all of the positive values in the set. Our sensitivity score was around **.74** which indicates the model falsely classified around a quarter of all the genuinely positive values.

- **F_meas or F1 Score:** This is the harmonic mean of precision and recall. Essentially it shows the balance between the two metrics in the model. Ours is around **.81** which indicates that, despite their values, the two scores are relatively balanced overall.

- **Specificity:** This shows the true negative rate. This shows how well we are at capturing all values that genuinely go against our null hypotheses. In our case this shows how well we catch all the customers who discontinue service. We got around **.79** which is okay, we miss around a fifth of the customers who churn which could use some improvement. 

- **roc_auc:** This the area under the roc curve. This curve has the true positive rate on the y-axis and the false positive rate on the x-axis. The closer to 1 the area under this curve is the better, and our model scores around a **.84** here which is respectable. 
